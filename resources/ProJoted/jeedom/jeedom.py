        ...
        try:
            ...
            while i < self.retry:
                try:
                    ...
                    logging.error(
                        "Error on send request to jeedom %s retry : %i/%i",
                        error,
                        i,
                        self.retry,
                    )
            ...
            if r.status_code != requests.codes.ok:
                logging.error(
                    "Error on send request to jeedom, return code %i", r.status_code
                )
    ...
    def find_tty_usb(idVendor, idProduct, product=None):
        ...
        for device in context.list_devices(subsystem="tty"):
            ...
            if product is not None:
                if (
                    "ID_VENDOR" not in device
                    or device["ID_VENDOR"].lower().find(product.lower()) == -1
                ):
                    continue
...
class jeedom_serial:
    ...
    def open(self):
        if self.device:
            logging.info(
                "Open serial port on device: %s, rate %s, timeout: %i",
                self.device,
                self.rate,
                self.timeout,
            )
        ...
        try:
            ...
            logging.error(
                "Error: Failed to connect on device %s. Details : %s", self.device, e
            )
            return False
